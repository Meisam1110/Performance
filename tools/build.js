#!/usr/bin/env node
/* Inline every asset into a single self-contained karaneh-hr.html so the user
 * can double-click it and run the system with no server and no network.
 *
 * The files division heads work in are not built here: HR produces them from
 * inside this one (پرسنل → تولید فایل معاون بخش). Each is a full copy of the
 * page — every screen, every setting — carrying one group's people and its own
 * storage slot.
 *   node tools/build.js
 */
'use strict';
...
var html = [
  ...
  '',
  '  This is the file HR works in. Division-head files are produced from inside',
  '  it (پرسنل → تولید فایل معاون بخش): each is a full copy of this page with',
  '  the same screens and settings, carrying only that group\'s people.',
  '',
...
var out = path.join(ROOT, 'karaneh-hr.html');
