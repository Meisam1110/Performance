...
(function () {
  ...
  VIEWS.employees = function (main) {
    ...
    if (App.state.employees.length) {
      main.appendChild(U.card(
        isAdmin() ? 'تحویل فایل و تفکیک پرسشنامه' : 'تفکیک پرسشنامه بین مدیران',
  ...
  VIEWS.help = function (main) {
    ...
    if (isAdmin()) {
      ...
      steps.appendChild(stepCard(3, 'برای هر معاون یک فایل بسازید', [
        'در «پرسنل» گزینهٔ «تولید فایل معاون بخش» را بزنید و مبنای تفکیک را انتخاب کنید.',
        'هر معاون یک فایل مستقل با پرسنل خودش می‌گیرد و در نقش معاون بخش باز می‌شود.',
