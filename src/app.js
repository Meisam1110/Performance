...
(function () {
  ...
  function boot() {
    ...
    Store.read().then(function (saved) {
      ...
      var repaired = repairRecordKeys();
      if (repaired) {
        Store.audit(App.state, {
          entity: 'questionnaire', field: '_key', oldValue: 'تکراری',
          newValue: repaired + ' رکورد', reason: 'اصلاح کلیدهای تکراری رکوردها'
        });
        save();
      }
      renderShell();
   ...
   * Record keys
   *
   * Every questionnaire row carries a `_key`, and every edit — a ticked
   * approval, a changed answer, a set amount — finds its record by it. Two
   * records sharing a key means an edit lands on the wrong person: you tick
   * one row and another lights up.
   *
   * The counter used to restart from the number of records, so after any
   * removal the next import handed out a key that was still in use. It is
   * taken from the highest key actually present instead, and every new key is
   * checked against the ones already out.
   * ====================================================================*/
  var keysInUse = null;

  function resetRecordKeys() {
    keysInUse = null;
    var high = 0;
    (App.state.questionnaires || []).forEach(function (q) {
      var m = /^q(\d+)$/.exec(String(q._key || ''));
  ...
  function nextRecordKey() {
    var used = recordKeySet(), n = App._keySeq || 0, key;
    do { key = 'q' + (++n); } while (used[key]);
    App._keySeq = n;
    used[key] = 1;
    return key;
  }
  ...
  /**
   * Give any record a key of its own.
   * Older saved files can already hold duplicates from the counter bug, and
   * they stay broken until the keys are made unique again.
   */
  function repairRecordKeys() {
    resetRecordKeys();
    var seen = {}, fixed = 0;
    (App.state.questionnaires || []).forEach(function (q) {
      if (!q._key || seen[q._key]) {
        q._key = nextRecordKey();
        fixed++;
      }
      seen[q._key] = 1;
      recordKeySet()[q._key] = 1;
    });
    return fixed;
  }
  ...
  VIEWS.import = function (main) {
    ...
    /* Questionnaire files and personnel files live in separate tables.
       Mixed into one list they were a row apart, and «حذف» on a personnel file
       throws away the whole roster — too easy to hit by mistake. */
    var batches = App.state.importBatches || [];
    var kinds = [
      { kind: 'questionnaire', title: 'فایل‌های پرسشنامه', key: 'batchesQ',
        hint: 'حذف یک فایل پرسشنامه، فقط پاسخ‌های همان فایل را برمی‌دارد' },
      { kind: 'employee', title: 'فایل‌های پرسنل', key: 'batchesE',
        hint: 'حذف یک فایل پرسنل، رکوردهای پرسنلی همان فایل را برمی‌دارد' },
      { kind: 'amounts', title: 'فایل‌های مبالغ', key: 'batchesA',
        hint: 'مبالغی که از فایل وارد شده‌اند' }
    ];

    kinds.forEach(function (group) {
      var rows = batches.filter(function (b) { return (b.kind || 'questionnaire') === group.kind; });
      if (!rows.length) return;
      var grid = U.DataGrid({
        title: group.title + ' (' + rows.length + ')', stateKey: group.key,
        rows: rows.slice().reverse(),
        searchFields: ['fileName', 'sheetName'],
        pageSize: 50,
        rowClass: function () { return group.kind === 'employee' ? 'row-employee-file' : ''; },
        columns: [
          { key: 'fileName', label: 'نام فایل', width: '220px' },
      ...
      var card = el('div', {});
      card.appendChild(el('div', { class: 'small muted', style: 'margin:14px 0 4px' },
        [document.createTextNode(group.hint)]));
      card.appendChild(grid.node);
  ...
  function commitAmounts(parsed) {
    /* No amount column, no import. Without this the file looked accepted and
       every matched person had their amount wiped, because an empty cell means
       "back to the computed figure". */
    var noAmountColumn = parsed.filter(function (p) {
      return p.mapping.hodAdjustment === undefined && p.mapping.finalKaraneh === undefined;
    });
    if (noAmountColumn.length) {
      U.modal({
        title: 'ستون مبلغ پیدا نشد', size: 'narrow',
        content: el('div', {}, [
          el('p', {}, [document.createTextNode(
            'در ' + (noAmountColumn.length === parsed.length ? 'این فایل' : 'یکی از فایل‌ها') +
            ' ستونی که مبلغ را نگه دارد شناسایی نشد، بنابراین چیزی تغییر نکرد.')]),
          el('p', { class: 'small muted' }, [document.createTextNode(
            'سرستون مبلغ می‌تواند یکی از این‌ها باشد: «کارانه نهایی»، «مبلغ نهایی»، ' +
            '«تغییرات معاون بخش»، «مبلغ» یا Final Karaneh. ' +
            'ساده‌ترین راه، خروجی گرفتن از همین صفحه و پر کردن همان ستون است.')])
        ]),
    ...
    var applied = 0, cleared = 0, missing = [], noComment = 0, ineligible = 0;
    parsed.forEach(function (p) {
      p.records.forEach(function (rec) {
        ...
        /* Below the threshold the engine pays nothing, so a figure typed for
           such a person is stored but changes no payout. Say so. */
        if (value !== null && !row.eligible) ineligible++;
      });
    ...
    if (ineligible) {
      msg += ' ' + U.int(ineligible) + ' نفر واجد شرایط نیستند (امتیاز زیر حد نصاب)، ' +
             'مبلغشان ثبت شد اما در پرداخت اثری ندارد.';
    }
    if (missing.length) {
  ...
  function removeBatch(batch) {
    var what = batch.kind === 'employee' ? 'رکوردهای پرسنلی'
             : batch.kind === 'amounts' ? 'مبالغ' : 'پاسخ‌های پرسشنامه';
    U.confirm(what + ' واردشده از فایل «' + batch.fileName + '» حذف شوند؟ (' +
      U.int(batch.accepted || 0) + ' رکورد)',
      { danger: true, confirmLabel: 'حذف' }).then(function (ok) {
      if (!ok) return;
  ...
  VIEWS.questionnaires = function (main) {
    ...
    var bulkPanel = bulkAnswerPanel();
    main.appendChild(U.card('ویرایش گروهی پاسخ‌ها', bulkPanel,
    ...
    var grid = U.DataGrid({
      ...
      onRender: function () { if (bulkPanel.refresh) bulkPanel.refresh(); },
  ...
  function bulkAnswerPanel() {
    ...
    /* Off by default: the common job is changing answers that are already
       there. Left on, the panel silently targeted nothing on a fully answered
       sheet and the button looked broken. */
    var onlyBlank = el('input', { type: 'checkbox' });
    onlyBlank.checked = false;

    var count = el('span', { class: 'small muted' });

    function visibleRows() {
      var grid = App.grids.questionnaires;
      return grid ? grid.getVisibleRows() : App.result.rows;
    }
    ...
    /** Why the target set is empty — a count of zero on its own explains nothing. */
    function emptyReason() {
      if (!visibleRows().length) return 'جدول پایین با فیلترهای فعلی ردیفی نشان نمی‌دهد.';
      if (fieldSel.value === '__impactApproved') {
        return 'در ردیف‌های نمایش‌داده‌شده کسی اثرگذاری ویژه ادعا نکرده است.';
      }
      if (onlyBlank.checked) {
        return 'در ردیف‌های نمایش‌داده‌شده، این ستون جای خالی ندارد — تیک «فقط خانه‌های خالی» را بردارید.';
      }
      return 'ردیفی برای تغییر وجود ندارد.';
    }

    function refreshCount() {
      var n = targets().length;
      count.textContent = n
        ? U.int(n) + ' ردیف هدف این تغییر است.'
        : emptyReason();
      count.className = 'small ' + (n ? 'muted' : 'warn-text');
    }
    ...
    box.appendChild(el('div', { style: 'display:flex;gap:8px;align-items:center;flex-wrap:wrap' }, [
      btn('اعمال روی ردیف‌های فیلترشده', function () {
        applyBulkAnswers(fieldSel.value, valueSel.value, targets(), emptyReason());
      }, 'primary'),
      count
    ]));
    refreshCount();
    /* The table below is rebuilt after this panel; whenever it repaints — a
       filter, a search, a page — it calls back so the count stays true. */
    box.refresh = refreshCount;
    return box;
  }

  function applyBulkAnswers(field, value, rows, reason) {
    if (!rows.length) {
      U.toast(reason || 'ردیفی برای تغییر وجود ندارد.', 'warn', 7000);
      return;
    }
  ...
  var BULK_MODES = [
    { key: 'fixed',   label: 'همه یک مبلغ یکسان بگیرند',
      amountLabel: 'مبلغ هر نفر (ریال)',
      say: function (n, amount) {
        return 'هر ' + U.int(n) + ' نفر این گروه ' + U.money(amount) + ' ریال می‌گیرند.';
      } },
    { key: 'percent', label: 'درصدی از دریافتی محاسباتی هر نفر',
      amountLabel: 'درصد (مثلاً ۱۲۰)',
      say: function (n, amount) {
        return 'دریافتی هر نفر ' + U.score(amount, 0) + '٪ مبلغ محاسباتی خودش می‌شود — ' +
               'کسی که بیشتر گرفته، بیشتر می‌گیرد.';
      } },
    { key: 'pool',    label: 'یک مبلغ کل بین گروه، به نسبت امتیاز',
      amountLabel: 'مبلغ کل گروه (ریال)',
      say: function (n, amount) {
        return U.money(amount) + ' ریال بین ' + U.int(n) +
               ' نفر به نسبت امتیاز کل هرکس تقسیم می‌شود.';
      } },
    { key: 'clear',   label: 'حذف مبلغ‌های دستی و بازگشت به محاسبه',
      amountLabel: '—',
      say: function (n) {
        return 'مبلغ دستی ' + U.int(n) + ' نفر پاک می‌شود و دریافتی آن‌ها دوباره از ' +
               'محاسبه می‌آید.';
      } }
  ];

  function bulkMode(key) {
    return BULK_MODES.filter(function (m) { return m.key === key; })[0] || BULK_MODES[0];
  }
  ...
  function bulkAmountPanel() {
    ...
    var amountLabel = el('span', { text: bulkMode(state.mode).amountLabel });
    var reason = el('input', { type: 'text', class: 'editable', style: 'width:100%',
      placeholder: 'دلیل این تغییر گروهی — الزامی' });
    var explain = el('div', { class: 'small', style: 'margin:4px 0 0;color:var(--text-2)' });
    var preview = el('div', { class: 'small', style: 'margin:10px 0' });
    var whoBox = el('div', { class: 'small muted', style: 'margin:6px 0 0' });
    ...
    function renderPreview() {
      ...
      if (!rows.length) {
        preview.appendChild(el('span', { class: 'muted',
          text: state.field === '__filtered'
            ? 'جدول پرداخت با فیلترهای فعلی ردیفی نشان نمی‌دهد.'
            : 'هنوز گروهی انتخاب نشده است.' }));
        return;
      }

      /* Who «the group» actually is, by name, so nothing is applied blind. */
      var names = rows.slice(0, 4).map(function (r) { return r.fullName || r.employeeId; });
      whoBox.appendChild(document.createTextNode(
        'این گروه: ' + names.join('، ') +
        (rows.length > names.length ? ' و ' + U.int(rows.length - names.length) + ' نفر دیگر' : '')));

      if (state.mode !== 'clear' && !(Number(state.amount) > 0)) {
        explain.appendChild(el('b', { text: 'عدد را وارد کنید تا اثرش را ببینید.' }));
        return;
      }
      explain.appendChild(el('b', { text: mode.say(rows.length, Number(state.amount) || 0) }));

      var plan = bulkPlan(rows, state.mode, state.amount);
      var before = 0, after = 0, unchanged = 0;
      plan.forEach(function (p) {
        before += p.row.finalKaraneh || 0;
        after += p.value === null ? (p.row.initialAllocation || 0) : p.value;
        var now = p.row.isOverridden ? p.row.hodAdjustment : null;
        if (String(now === null ? '' : now) ===
            String(p.value === null ? '' : Math.round(p.value))) unchanged++;
      });

      preview.appendChild(el('div', {}, [
        el('b', { text: U.int(rows.length - unchanged) + ' نفر تغییر می‌کنند' }),
        unchanged ? el('span', { class: 'muted',
          text: '  (' + U.int(unchanged) + ' نفر همین‌الان همین مبلغ را دارند)' }) : null,
        document.createTextNode('  •  مجموع این گروه: '),
      ...
      if (Math.abs(delta) >= 1) {
        preview.appendChild(el('div', { class: 'muted', style: 'margin-top:3px' }, [
          document.createTextNode(delta > 0
            ? 'این اختلاف از سهم سایر افراد خارج این گروه کم می‌شود: '
            : 'این اختلاف به سهم سایر افراد خارج این گروه اضافه می‌شود: '),
    ...
    box.appendChild(el('div', { class: 'form-grid' }, [
      ...
      el('label', { class: 'field' }, [amountLabel, amountInput])
    ]));
    box.appendChild(explain);
    box.appendChild(whoBox);
    box.appendChild(el('label', { class: 'field', style: 'margin-top:10px' },
      [el('span', { text: 'دلیل' }), reason]));
  ...
  function applyBulkAmounts(state, reason) {
    ...
    U.confirm('مبلغ ' + U.int(plan.length) + ' نفر تغییر کند؟ این کار سهم سایر افراد را جابه‌جا می‌کند.',
      ...
      save();
      var check = Engine.validateBudget(recalc());
      var skipped = plan.length - changed;
      var msg = U.int(changed) + ' نفر از ' + U.int(plan.length) + ' نفر گروه تغییر کردند' +
                (skipped ? '؛ ' + U.int(skipped) + ' نفر از قبل همین مبلغ را داشتند' : '') + '.';
      if (!check.ok) {
        U.toast(check.problems[0].message, 'err', 9000);
      } else {
        U.toast(msg, 'ok', 6000);
      }
    });
  ...
  /* Test hooks for the record-key repair. */
  window.__repairKeys = function () { return repairRecordKeys(); };
  window.__byKey = function (k) { return questionnaireByKey(k); };
  window.__editField = function (q, f, v, r) { return editField(q, f, v, r); };
